...
import { IconDatabase, IconSearch, IconPlus } from '@tabler/icons-react';
import { useMemo, useState } from 'react';
import { dummyAiApps } from '@/lib/dummyData';

const AiAppsPage = () => {
  const [query, setQuery] = useState('');
  const apps = dummyAiApps;
          ...
          <div className="grid grid-cols-12 gap-6 h-full">
              ...
              <div className="flex-1 overflow-auto divide-y divide-gray-100">
                {filtered.map(a => (
                  <div key={a.id} className="p-4 hover:bg-gray-50 cursor-pointer flex items-center justify-between">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${a.status === 'Enabled' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'}`}>{a.status}</span>
                  </div>
                ))}
                {filtered.length === 0 && <div className="p-8 text-center text-sm text-gray-500">No AI apps found.</div>}
              </div>
            </div>
            <div className="col-span-8 bg-white border border-gray-200 rounded-xl h-full flex flex-col">
              ...
              <div className="flex-1 overflow-auto p-6 text-sm text-gray-700">
                Select an AI app to configure behaviors, triggers, and outputs.
              </div>
            </div>
          </div>
        </div>
